...
def freqs_cis_table(dim,content_length=100,theta=10000.0):
    freqs=1.0/(theta**(torch.arange(0,dim,2).float()/dim))
    position=torch.arange(0,content_length,1).unsqueeze(1)
    position_lookup_table=position*freqs
...
def compute_position(Q,K,position_lookup_table):
    Q=rearrange(Q,'b t h (pair complex) -> b t h pair complex',complex=2)
    K=rearrange(K,'b t h (pair complex) -> b t h pair complex',complex=2)
    Q=torch.view_as_complex(Q.float())
    K=torch.view_as_complex(K.float())
    current_length=Q.shape[1]
    position_lookup_table=position_lookup_table[:,:current_length,:]
    position_lookup_table=rearrange(position_lookup_table,'B T (h C) -> B T h C',h=1).to(device)
    Q=Q*position_lookup_table
    K=K*position_lookup_table
    Q=torch.view_as_real(Q)
    K=torch.view_as_real(K)
    Q=rearrange(Q,'b t h pair complex -> b t h (pair complex)')
    K=rearrange(K,'b t h pair complex -> b t h (pair complex)')
...
class Multi_Attention(nn.Module):
    def __init__(self, d_model,batch_size,num_heads, bias=False):
        ...
        cis_table=freqs_cis_table(self.heads_div,content_length=100)
        self.register_buffer('cis_table',cis_table)
    ...
    def forward(self,x):
        ...
        Q=Q.view(x.shape[0],x.shape[1],self.num_heads,self.heads_div)
        K=K.view(x.shape[0],x.shape[1],self.num_heads,self.heads_div)
        V=V.view(x.shape[0],x.shape[1],self.num_heads,self.heads_div).permute(0,2,1,3)

        Q,K=compute_position(Q,K,self.cis_table)
