...
class TRPOAgent:
	...
	def get_policy_loss(self, states, actions, advantages, old_probs):
		probs = self.actor(states)
		dist = Categorical(probs)
		log_probs = dist.log_prob(actions)
		old_action_probs = old_probs.gather(1, actions.view(-1, 1)).squeeze(1)
	...
	def fisher_vector_product(self, states, old_probs, vector):
	...
	def conjugate_gradient(self, states, old_probs, b):
		...
		for _ in range(self.cg_iters):
			Hp = self.fisher_vector_product(states, old_probs, p)
	...
	def line_search(self, states, actions, advantages, old_probs, step_dir, old_params, old_loss):
		sHs = torch.dot(step_dir, self.fisher_vector_product(states, old_probs, step_dir))
		if sHs <= 0:
		...
		for i in range(self.backtrack_iters):
			...
			new_loss = self.get_policy_loss(states, actions, advantages, old_probs)
			kl = self.get_kl(states, old_probs)
	...
	def update(self):
		...
		old_loss = self.get_policy_loss(states, actions, advantages, old_probs)
		grads = torch.autograd.grad(old_loss, self.actor.parameters())
		flat_grads = self.flat_grad(grads).detach()

		step_dir = self.conjugate_gradient(states, old_probs, flat_grads)
		old_params = self.flat_params(self.actor).detach()
		self.line_search(states, actions, advantages, old_probs, step_dir, old_params, old_loss)
...
while total_steps < 100000:
	...
	while not done:
		...
		if len(agent.data) >= min_batch_size:
			agent.update()
			print(f"Steps: {total_steps}, Last Episode Reward: {episode_reward}")
