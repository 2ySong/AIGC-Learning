...
# 1. 创建一个新的环境，专门用来展示，开启 render_mode="human"
# 注意：如果你在没有屏幕的服务器上，这一步会报错，请跳过
display_env = gym.make('CartPole-v1', render_mode="human")
...
while not done:
    # 2. 这里的动作选择完全依赖网络（不再探索），只选 Q 值最大的
    # 因为我们想看它"最强"的样子
    action = agent.select_action(state) 
    
    # 注意：如果是为了展示，这里最好强制把 epsilon 设为 0
    # 但由于我们的 select_action 里用了 epsilon，你可以临时改一下
    # 或者相信现在的 epsilon (0.09) 已经足够小了
    
    next_state, reward, terminated, truncated, _ = display_env.step(action)
    done = terminated or truncated
    state = next_state
