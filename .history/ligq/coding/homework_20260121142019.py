...
def gengerate_token(model,start_token,max_token_num):
    model.eval()
    index=start_token
    with torch.no_grad():
        for _ in range(max_token_num):
            ...
            index=torch.concat((index,token_next),dim=-1)
    return index
...
for step in range(max_iter):
    real_input,real_output=get_batch(train_data,content_length=16,batch_size=4)
...
gengerate_result=gengerate_token(model,content,max_token_num=50)
gengerate_result=tokenizer.decode(gengerate_result[0].tolist())
print(gengerate_result)
